...
const Navbar: React.FC<NavbarProps> = ({ forceOpaque = false }) => {
  ...
  return (
    <nav 
      ref={navRef} 
      className={`fixed top-0 left-0 right-0 z-50 px-4 md:px-6 py-2 transition-all duration-700 ${
        isNavOpaque 
        ...
        <div className="flex-1 lg:flex-none flex items-center overflow-visible">
          <Link to="/" className="flex items-center space-x-2 md:space-x-3 group cursor-pointer transition-transform duration-300">
            ...
            <h1 className={`text-[0.65rem] md:text-[0.75rem] uppercase tracking-[0.15em] md:tracking-[0.2em] font-medium transition-colors duration-300 whitespace-nowrap ${isNavOpaque ? "text-foreground group-hover:text-primary" : "text-white group-hover:text-white/80"}`}>
              Bella Vita <span className="opacity-80 hidden xs:inline">Medi Spa</span>
            </h1>
          </Link>
        </div>
        ...
        <div className="hidden lg:flex items-center justify-center space-x-10">
          {navLinks.map((link) => (
            <Link 
              key={link.title}
              to={link.href} 
              className={`relative transition-all duration-300 text-sha-caps group ${
            ...
            >
              ...
              <span className={`absolute -bottom-1 left-0 h-[1.5px] transition-all duration-300 ${
                (location.pathname === link.href) ? "w-full bg-gradient-to-r from-primary to-accent" : "w-0 group-hover:w-full " + (isNavOpaque ? "bg-gradient-to-r from-primary to-accent" : "bg-white")
              }`}></span>
            </Link>
          ))}
        </div>
